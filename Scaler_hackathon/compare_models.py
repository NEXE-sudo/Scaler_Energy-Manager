...
def print_comparison(base_results, lora_results, task_ids):
    ...
    for task_id in task_ids:
        ...
        l_score = l.get("score", None)
        b_avg   = b.get("avg_reward", None)
        l_avg   = l.get("avg_reward", None)

        # Compute delta only if both scores are valid numbers
        if b_score is not None and l_score is not None and not math.isnan(b_score) and not math.isnan(l_score):
        ...
        print(f"  {task_id.upper():<10} {'Score':>{COL-9}} {b_score if b_score is not None else 'N/A':>8}    {'Score':>{COL-9}} {l_score if l_score is not None else 'N/A':>8}  {arrow} {abs(delta) if delta is not None else 'N/A'}")
        print(f"  {'':10} {'AvgRew':>{COL-9}} {b_avg if b_avg is not None else 'N/A':>8}    {'AvgRew':>{COL-9}} {l_avg if l_avg is not None else 'N/A':>8}")
        print(f"  {'':10} {'Blackout':>{COL-9}} {str(b.get('blackout', '?')):>8}    {'Blackout':>{COL-9}} {str(l.get('blackout', '?')):>8}")
        print(SEP)

        if b_score is not None and not math.isnan(b_score):
            total_base_score += b_score
            count += 1
        if l_score is not None and not math.isnan(l_score):
...
def main():
    ...
    if not args.skip_base:
        ...
        with open(args.base_scores, encoding="utf-8") as f:
            base_results = json.load(f)
        print(f"[INFO] Loaded base scores from {args.base_scores}")

    # Save base scores for reuse
    if base_results:
        with open("base_scores.json", "w", encoding="utf-8") as f:
    ...
    if lora_results:
        with open("lora_scores.json", "w", encoding="utf-8") as f:
    ...
    if base_results is None and Path("base_scores.json").exists():
        with open("base_scores.json", encoding="utf-8") as f:
            base_results = json.load(f)

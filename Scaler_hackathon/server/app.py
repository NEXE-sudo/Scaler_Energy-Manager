...
# Load blog content
blog_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "Blog.md")
...
# We check if the directory exists to avoid crashes if the frontend hasn't been built yet
frontend_dist = os.path.join(os.path.dirname(os.path.dirname(__file__)), "agent-grid-view", "dist")
if os.path.exists(frontend_dist):
    ...
    async def root():
        return {
            "message": "API running (Frontend not built. Run 'npm run build' in agent-grid-view)",
        }
